...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            font_family: super::terminal::FONT_FAMILY.to_string(),
            font_size: super::terminal::DEFAULT_FONT_SIZE,
            option_as_alt: true,
...
fn migrate_from_json(config_dir: &std::path::Path) -> Option<(String, String)> {
    ...
    Some((
        ...
        legacy
            .font_family
            .unwrap_or_else(|| super::terminal::FONT_FAMILY.to_string()),
    ))
}
    ...
    if !path.exists() {
        ...
        let (theme, font) = migrate_from_json(parent).unwrap_or_else(|| {
            (
                ...
                super::terminal::FONT_FAMILY.to_string(),
            )
        });
...
pub fn load_config() -> Config {
    ...
    // Size guard
    if content.len() > super::settings::MAX_FILE_SIZE as usize {
        tracing::warn!(
